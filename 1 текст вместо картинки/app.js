...
function handleClick(clickedButtonEvent) {
    ...
    let currentBtnText = card.button.innerText;
    if (currentBtnText == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (currentBtnText == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let someText = 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quam, officia?';
